...
### 이슈가 뭔가 — 레포에 붙는 "할 일 카드"

이슈(Issue)는 코드가 아니라 **"무엇을, 왜 할지"를 적어두는 카드**임. 만들면 자동으로 `#1, #2…` 번호가 붙고, 제목·설명·담당자(assignee)·라벨·상태(open/closed)를 가짐.

> 지난주엔 그냥 브랜치 파고 작업했음. 근데 사람이 여럿이면 "누가 지금 뭐 하지?", "이건 대체 왜 바꿨지?"가 안 보임. **작업을 하기 전에 이슈로 먼저 선언**해두면 이게 다 해결됨.

- **분업** — Assignee를 지정하면 누가 뭐 하는 중인지 이슈 목록에서 한눈에
- **분류** — Label(레포에 이미 정의돼 있음)을 붙이면 종류별로 필터링
- **추적** — 커밋·PR이 이 이슈에 자동으로 모여서 "왜 했는지"가 남음

### #N — 레포 안의 주소

이슈와 PR은 **같은 번호 공간**을 씀. 이슈 만들면 `#3`, 다음에 열리는 PR은 `#4`… 이 번호를 어디에 적든 GitHub이 자동으로 링크를 걸고 이슈 타임라인에 기록함.

| 어디에 쓰나 | 예시 | 효과 |
|---|---|---|
| 커밋 메시지 | `feat: 주간 로그 추가 (#3)` | 이슈 #3 타임라인에 커밋이 표시됨 (닫히진 않음) |
| PR 본문 | `Closes #3` | 이 PR이 merge되는 **순간 #3 자동 close** ✨ |
| 코멘트 / 다른 이슈 | `#3` | 서로 링크 걸림 |

> ⚠️ 자동 close는 **`Closes` / `Fixes` / `Resolves` + 번호**만 인식하고, **PR 본문**에 있어야 함. 커밋 메시지나 코멘트에 쓰면 링크만 되고 닫히진 않음.

### 전체 흐름 — 이슈가 먼저, 작업은 그다음

```
Issue 생성(#3) → 브랜치 feat/3-week02-practice → 커밋 (#3) → PR "Closes #3" → 리뷰 → merge → 이슈 자동 close
```

지난주에 배운 브랜치/커밋/PR이 전부 이 `#3` 하나로 꿰어지는 게 핵심임.

### 🛠️ 실습 — 페어 이슈 만들기 (STEP by STEP)

아래 3번 conflict 실습에서 쓸 이슈를 **지금** 만들어 둠. 페어당 이슈 1개.

**STEP 1 — New issue 열기**

레포 상단 `Issues` 탭 → 초록색 `New issue`. 템플릿 목록이 뜨면 `📅 주간 과제 (Weekly Task)`의 `Get started`(실습이니 맨 아래 `Blank issue`로 해도 됨).

**STEP 2 — 제목 (title)** — 제목만 봐도 뭔지 알게, 한 줄로
...
**STEP 3 — 본문 (description)** — 뭘 할 건지 2~3줄

```
같은 파일 같은 줄을 둘이 다르게 고쳐서 conflict를 일부러 내고 터미널로 해결한다.
- [ ] 선공 PR merge
- [ ] 후공 conflict 해결 후 merge
```

> 💡 `- [ ]` 는 GitHub에서 체크박스로 렌더됨 (작업 목록).

**STEP 4 — 오른쪽 사이드바 설정**

- **Assignees**: 페어 **두 명 다** 지정 (기어 아이콘 → 이름 검색)
- **Labels**: `weekly-task` 붙이기 (레포에 이미 정의돼 있음)
- **Milestone**: `Week 2`가 있으면 선택, 없으면 스킵

**STEP 5 — 제출 & 번호 확인**

`Create`(Submit new issue) → 제목 옆에 `#N`이 붙음.

> ✅ `Issues` 탭에 방금 만든 이슈가 **open**으로 보이면 성공.
> 📌 **이 `#N`을 각자 메모.** 아래 3번에서 브랜치(`feat/N-conflict-이름`)와 PR(`Closes #N`)에 계속 씀.
